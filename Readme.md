...
适配器模式有两种实现方式：**类**适配器【基于继承，继承旧，实现新，里面用旧】和**对象**适配器【基于组合，持有旧，实现新，里面用旧】。
...
### 2.5 门面
为一组**复杂的子系统提供了一个简单的接口**，使得**子系统更容易使用和理解**。
**门面模式通常在以下情况下使用：**

1. **将复杂的子系统进行抽象和封装**：当一个系统变得复杂时，往往会由**多个子系统组成**。门面模式可以将这些子系统进行抽象和封装，提供一个简单的接口供客户端使用。
2. **隐藏子系统的复杂性**：将子系统的**复杂性隐藏起来**
3. **提供一个简单的接口**：为客户端提供一个简单的接口
4. **降低客户端与子系统的耦合**：将客户端和子系统解耦

【像简单工厂一样，门面把所有类都整合在一起，统筹向外提供服务，而简单工厂把所有产品整合在一起，统筹提供产品】

**适配器模式和门面模式区别**：
* 适配器模式为了解决接口不兼容性问题，门面模式提供一个统一的高级接口，隐藏子系统的复杂性
* 适配器模式通常只包装一个类或对象，将一个接口转换为另一个接口。门面模式包装了一整个子系统或者一组接口，将一系列复杂操作封装成一个简单的接口。

### 2.6 组合
将一组对象组织（Compose）成树形结构，以表示一种“部分 - 整体”的层次结构。
**每个对象都有相同的接口**，客户端**不需要知道对象的具体类型**，而只需要调用对象的**通用接口**即可。

1. **Component（抽象构件）**：组合对象的通用接口，可以包含其他组合对象或叶子对象。
2. **Composite（组合节点）**：组合对象中的组合节点，可以包含其他组合对象或叶子对象。
3. **Leaf（叶子节点）**：组合对象中的叶子节点，没有子节点。

仓库中的例子是简单的文件夹及文件的展示系统。

### 2.7 享元 
被共享的单元。意图是复用对象，节省内存，前提是享元对象是不可变对象。

两种情形
* 当系统中存在**大量不可变的重复对象**，在内存中只保留一份实例，供多处代码引用。
* 对于相似对象，也可以将这些**对象中相同的部分（字段）**提取出来，设计成享元，让这些大量相似对象引用这些享元。

仓库中里的例子为棋局大厅，棋子的id，名称、颜色是共享的，坐标不一致。

享元模式跟对象池的区别：
* 对象池、连接池（比如数据库连接池）、线程池等也是为了复用，主要目的是**节省时间**。
...
### 3.5 状态
允许对象在**其内部状态发生变化时改变其行为**。这种模式可以**消除大量的条件语句**，并将每个状态的行为封装到单独的类中。

主要组成部分：
1. **上下文（Context）**：通常包含一个**具体状态的引用**，用于维护当前状态。上下文委托给当前状态对象处理状态相关行为。
2. **抽象状态（State）**：一个**接口，用于封装与上下文的特定状态相关的行为**。
3. **具体状态（Concrete State）**：**实现抽象状态接口**，为具体状态定义行为。每个具体状态类对应一个状态。

TV代码示例中，传统的TV类想调整音量，得用if-else判断是否开机，想关机，也得用if-else判断是不是开机状态。
这样实现的功能越多，对其他状态的判断也就越来越多。因此，需要做出改动。

每一个修改tv状态的动作抽象成一个接口， 当然接口中也包括调整音量等其他能力。
实现该接口时，核心状态有两个，开机状态下和关机状态下。
对于一个TV，需持有一个TVState，初始化的tv为关机状态。然后可以自行执行接口中的方法。

#### 有限状态机
有限状态机是描述对象在它的生命周期内所经历的状态序列，以及如何响应来自外界的各种事件**。。状态机有 3 个组成部分：状态（State）、事件（Event）、动作（Action）。
其中，事件也称为转移条件（Transition Condition）。事件触发状态的转移及动作的执行。不过，动作不是必须的，也可能只转移状态，不执行任何动作。

为超级马里奥为例，人物有小、大、火焰、死亡四种状态，导致变化的契机有吃蘑菇、吃花、被小兵攻击、其他致死（掉坑）4种方式。
* 分支法：每个状态下，遇到什么事，有什么结果，if-else或switch-case
* 查表：4x4的状态转移表，在某个状态，遇到某事，直接在表中查到最终状态

| 当前状态/事件 | MUSHROOM | FIRE_FLOWER | ENEMY_ATTACK | FALL_INTO_PIT |
| ------------- | -------- | ----------- | ------------ | ------------- |
| SMALL         | BIG      | SMALL       | DEAD         | DEAD          |
| BIG           | BIG      | FIRE        | SMALL        | DEAD          |
| FIRE          | FIRE     | FIRE        | SMALL        | DEAD          |
| DEAD          | DEAD     | DEAD        | DEAD         | DEAD          |

* 状态模式：改变状态的事件都抽象为接口，每个状态都实现这些接口，写相应的代码

### 3.6 迭代器
用于遍历集合对象，而无需**暴露该对象的底层表示**。
这种模式非常适合在**处理大型集合**时使用，因为它提供了一种**更抽象的方式来访问集合的元素**。

几个关键组件：

1. **Iterator接口**：遍历集合所需的操作，例如next()、hasNext()等。
2. **ConcreteIterator类**：实现Iterator接口，具体实现遍历的细节。
3. **Aggregate接口**：创建Iterator对象的方法。
4. **ConcreteAggregate类**：实现Aggregate接口，具体实现创建ConcreteIterator的方法。

仓库代码为【基于迭代器模式的整数数组遍历示例】
* 标准流程，就是那4个一个都不少
* 内部类，有点像建造者模式中，抽象建造者移除，具体建造者成为产品的内部类，同时内部吃持有外部产品
  * 这里是具体接口实现类移除，成为IntArray的内部实现了迭代器接口的类
